...
<body>
    <div class="container">
        <div class="header">
            <h1>🏥 Sistema de Registro Médico</h1>
        ...
        <div id="patient-section" class="content-section">
            ...
            <div class="instructions">
                <strong>Instrucciones:</strong> Complete el formulario con sus datos. Su información será registrada de forma segura.
            </div>
        ...
        <div id="staff-section" class="content-section">
            ...
            <div id="staff-content" style="display: none;">
                <div class="instructions">
                    <strong>Personal autorizado:</strong> Registre pacientes durante las consultas o citas.
                </div>
    ...
    <script>
        ...
        const STAFF_PASSWORD = "Son1970ia"; // Nueva contraseña para personal médico
        const CONSULTATION_DURATION = 20; // Minutos por consulta
        const START_HOUR = 9; // Hora de inicio (9 AM)
        const END_HOUR = 17; // Hora de fin (5 PM)
        ...
        function calculateConsultationTime(turnNumber) {
            ...
            // Si es muy tarde, programar para el día siguiente
            if (now.getHours() >= END_HOUR) {
                consultationDate.setDate(consultationDate.getDate() + 1);
                consultationDate.setHours(START_HOUR, 0, 0, 0);
            } else {
                // Calcular tiempo basado en el número de turno
                const minutesFromStart = (turnNumber - 1) * CONSULTATION_DURATION;
                consultationDate.setHours(START_HOUR, minutesFromStart, 0, 0);
                
                // Si la hora calculada es anterior a ahora, usar la hora actual + buffer
                if (consultationDate < now) {
                    consultationDate.setTime(now.getTime() + (CONSULTATION_DURATION * 60 * 1000));
                }
            }
            
            return consultationDate.toLocaleTimeString('es-MX', {
                hour: '2-digit',
                minute: '2-digit',
                hour12: true
            });
        ...
        document.getElementById('patientForm').addEventListener('submit', function(e) {
            ...
            const estimatedTime = calculateConsultationTime(turnNumber);
            ...
            turnInfo.innerHTML = `
                ...
                <p><strong>Su número de turno:</strong> <span style="font-size: 1.5em; color: #4facfe; font-weight: bold;">#${turnNumber}</span></p>
                <p><strong>Hora estimada de consulta:</strong> <span style="color: #4facfe; font-weight: bold;">${estimatedTime}</span></p>
                <p><em>Por favor llegue 10 minutos antes de su hora estimada.</em></p>
            `;
        ...
        document.getElementById('staffForm').addEventListener('submit', function(e) {
            ...
            staffTurnInfo.innerHTML = `
                ...
                <strong>Turno #${turnNumber}</strong> - Hora estimada: <strong>${estimatedTime}</strong>
            `;
        ...
        function displayPatientRecords(searchTerm = '') {
            ...
            container.innerHTML = filtered.map(patient => `
                <div class="record-item">
                    ...
                    <div class="record-date">${patient.displayDate} | Registrado por: ${patient.registeredBy}</div>
                    <div><strong>Teléfono:</strong> ${patient.phone || 'No proporcionado'}</div>
                    <div><strong>Email:</strong> ${patient.email || 'No proporcionado'}</div>
        ...
        function exportToCSV() {
            const headers = ['Nombre', 'Teléfono', 'Email', 'Motivo', 'Prioridad', 'Registrado por', 'Fecha'];
            const csvContent = [
                headers.join(','),
